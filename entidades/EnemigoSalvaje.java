...
public class EnemigoSalvaje extends Enemigo implements Vulnerable {
    ...
    public EnemigoSalvaje(Tipo tipo) {
        ...
        switch (tipo) {
            case PLANTA_CARNIVORA:
                ...
                this.stats = new Estadisticas(120, 0, 18, 0);
                break;
            case SAPO_JUNGLA:
                this.nombre = "Sapo de la Jungla";
                this.stats = new Estadisticas(90, 0, 22, 0);
                break;
            case ROBOT_CENTINELA:
                this.nombre = "Robot Centinela";
                this.stats = new Estadisticas(150, 0, 25, 0);
                break;
        }
    }
    ...
    public double evaluarDebilidad(Elemento elementoMagia) {
        switch (tipo) {
            case PLANTA_CARNIVORA:
                // Debil al fuego, resiste hielo, inmune a cura
                if (elementoMagia == Elemento.FUEGO) return 2.0;
                if (elementoMagia == Elemento.HIELO) return 0.5;
                if (elementoMagia == Elemento.CURA)  return 0.0;
                return 1.0;
            case SAPO_JUNGLA:
                // Debil al rayo, resiste fuego, normal al resto
                if (elementoMagia == Elemento.RAYO)  return 2.0;
                if (elementoMagia == Elemento.FUEGO) return 0.5;
                return 1.0;
            case ROBOT_CENTINELA:
                // Debil al rayo, inmune al fuego, resiste hielo
                if (elementoMagia == Elemento.RAYO)  return 2.0;
                if (elementoMagia == Elemento.FUEGO) return 0.0;
                if (elementoMagia == Elemento.HIELO) return 0.5;
                return 1.0;
            default:
                return 1.0;
